...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({
  ...
  const sidebarContent = (
    <div className="p-6 pt-24 lg:pt-8 md:p-8 h-full flex flex-col overflow-y-auto hide-scrollbar">
      ...
      <nav className="flex-grow space-y-1.5">
        <p className="text-[0.55rem] uppercase tracking-[0.2em] text-ink-faint font-body font-bold px-4 mb-3">{isBn ? 'হেলথ ড্যাশবোর্ড' : 'Health Dashboard'}</p>
        {navItems.map((item) => {
  ...
  return (
    <div className="h-screen h-[100dvh] bg-cream flex overflow-hidden font-bn relative">
      ...
      <aside className="hidden lg:flex w-[280px] bg-white border-r border-ink/5 flex-col shrink-0">
        {sidebarContent}
      </aside>
        ...
        {sidebarOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed inset-y-0 left-0 w-[280px] md:w-[320px] bg-white border-r border-ink/5 flex flex-col z-30 shadow-2xl lg:hidden"
            >
              {sidebarContent}
            </motion.aside>
          </>
          ...
          <div className="flex items-center gap-3 md:gap-5">
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-2.5 md:p-3 bg-cream rounded-2xl text-ink-muted hover:bg-ink hover:text-cream transition-all flex shadow-sm interactive lg:hidden"
              >
                {sidebarOpen ? <X size={20} /> : <Menu size={20} />}
              </button>
              <Link to="/" className="p-2.5 md:p-3 bg-cream rounded-2xl text-ink-muted hover:bg-ink hover:text-cream transition-all flex interactive">
                <ArrowLeft size={20} />
              </Link>
            </div>

            <div className="flex items-center gap-3 md:gap-4 transition-opacity duration-300">
              {headerExtra}
              <div>
